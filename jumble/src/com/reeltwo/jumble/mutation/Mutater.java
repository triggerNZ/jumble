...
public class Mutater {
  ...
  private boolean checkNormalMethod(final Method m, String className) {
    return m != null && !m.isNative() && !m.isAbstract() && !m.isSynthetic() && !isIgnored(m, className) && m.getName().indexOf("access$") == -1
      // This next one appears to be how the Eclipse compiler handles enums, it is
      // the equivalent of the synthetic class generated by javac
      && m.getName().indexOf("$SWITCH_TABLE$") == -1
    /* && m.getLineNumberTable() != null */ && m.getCode() != null
    /* && m.getLineNumberTable().getSourceLine(0) > 0; */
    && !isEnumAutoGeneratedMethod(m, className);
  }
  ...
  private boolean isEnumAutoGeneratedMethod(Method m, String className) {
    ...
    if (clazz.isEnum()) {
      ...
      if (m.getName().equals("valueOf") && m.getArgumentTypes().length == 1 && m.getArgumentTypes()[0].equals(org.apache.bcel.generic.BasicType.STRING)) {
        return true;
      }
    }
    return false;
