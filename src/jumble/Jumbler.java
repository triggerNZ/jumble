 ...
 * or selected at random. Uses a custom class loader.
 * 
...
public class Jumbler extends ClassLoader {
  ...
  /**
   * Construct a new loader which will induce a single point mutation in the
   * target.
   * 
   * @param target
   *          class name to undergo mutation
   * @param mutater
   *          mutation engine
   */
  public Jumbler(final String target, final Mutater mutater) {
    super();
    mTarget = target;
    mMutater = mutater;
  }
   ...
   */
  public JavaClass modifyClass(final JavaClass clazz) {
  ...
  /**
   * Process each of the supplied files in turn. Reports problems with variable
   * names.
   * 
   * @param args
   *          array of java file names
   */
  public static void main(String[] args) throws Exception {
    ...
    boolean constants = Utils.getFlag('k', args)
        || Utils.getFlag("inlineconstants", args);
    boolean returns = Utils.getFlag('r', args)
        || Utils.getFlag("returns", args);
    boolean increments = Utils.getFlag('i', args)
        || Utils.getFlag("increments", args);
    ...
    if (count) {
      ...
      System.out.println(""
          + m.countMutationPoints(Utils.getNextArgument(args)));
    } else {
      ...
      try {
        ...
        System.err.println(clazz.getMethod("run", new Class[] { String.class })
            .invoke(null, new Object[] { testName.replace('/', '.') }));
      } catch (Exception ex) {
        ex.printStackTrace();
      }
